...
<c:set var="viewName" value="editDeleteRoster"/>
<jsp:directive.include file="/inc/rosterMenu.jsp"/>

<div class="instructions">
	<c:out value="${msgs.explanation_delete_user_list}" />
</div>
...
<!-- Title Selection -->
<form method="post" id="updateRoster" action=index.htm?action=editRemovedUserList&sectionId=<c:out value='${sectionId}'/>&updateFlg=true>
	<table id="rosterList"  class="tablesorter lines sectionTable" cellspacing="0" cellpadding="0"  summary="deleteList" >
		<thead>
			<tr>
		...
		<tbody>
			<c:if test="${empty viewdeleteRosterList }">
				<tr><td style="white-space:nowrap" colspan="9"><c:out value="${msgs.no_target_roster_available }"/></td></tr>
			</c:if>
			<c:if test="${not empty viewdeleteRosterList }">
				<c:forEach var="roster" items="${viewdeleteRosterList }">
					<tr>
						...
						<td style="white-space:nowrap">
							<input type="checkbox" name="deleteUserIds" value="<c:out value="${roster.userEid }"/>" />
						</td>
					</tr>
				</c:forEach>
				</c:if>
